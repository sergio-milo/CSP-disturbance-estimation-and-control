...
class JDBayesianOptimization(Observable):
    """
    ...
    def __init__(self,
                 ...
                 normalize_gp_before_fitting=False,
                 measurement_noise_variance=1e-6,
                 ):
        self._random_state = ensure_rng(random_state)
        self._allow_duplicate_points = allow_duplicate_points
        self._normalize_gp_before_fitting = normalize_gp_before_fitting
        self._measurement_noise_variance = measurement_noise_variance
        self._queue = Queue()

        # Internal GP regressor
        self._gp = GaussianProcessRegressor(
            kernel=Matern(nu=2.5),
            alpha=self._measurement_noise_variance,
            normalize_y=self._normalize_gp_before_fitting,
        ...
        super(JDBayesianOptimization, self).__init__(events=DEFAULT_EVENTS)
    ...
    def plot_gp(self, x, figure_handle=False):

        if self._space.params.shape[1] > 1:
            print("Cannot plot more than 1-dimensional cost functions")
            return
        
        fig = plt.figure(figsize=(14, 8))
        steps = len(self.space)
        fig.suptitle(
        ...
        f_axis_handle = plt.subplot(gs[0])
        f_axis_handle.grid(linewidth=0.25)
        acq_axis_handle = plt.subplot(gs[1])
        acq_axis_handle.grid(linewidth=0.25)

        x_obs = self._space.params
        y_obs = self._space.target        
        mu, sigma = self._gp.predict(x, return_std=True)

        # f_axis_handle.plot(x, y, linewidth=3, label='Unknown objective')
        f_axis_handle.plot(x_obs.flatten(), y_obs, 'D', markersize=8, label=u'Observations', color='r')
        f_axis_handle.plot(x, mu, '--', color='k', label='Prediction')

        f_axis_handle.fill(np.concatenate([x, x[::-1]]), 
                np.concatenate([mu - 1.9600 * sigma, (mu + 1.9600 * sigma)[::-1]]),
            alpha=.6, fc='c', ec='None', label='95\% confidence interval')
        
        f_axis_handle.set_xlim((x.min(), x.max()))
        f_axis_handle.set_ylim((None, None))
        f_axis_handle.set_ylabel('$f(x)$', fontdict={'size':20})
        f_axis_handle.tick_params(axis='x', labelsize=18)
        f_axis_handle.tick_params(axis='y', labelsize=18)
        
        acq_fcn_vec = UtilityFunction(kind="ucb", kappa=5, xi=0).utility(x, self._gp, self._space.target.max())

        acq_axis_handle.plot(x, acq_fcn_vec, label='Acquisition Function', color='purple')
        acq_axis_handle.plot(x[np.argmax(acq_fcn_vec)], np.max(acq_fcn_vec), '*', markersize=15, 
                label=u'Next Best Guess', markerfacecolor='gold', markeredgecolor='k', markeredgewidth=1)
        # f_axis_handle.plot(x[np.argmax(acq_fcn_vec)], y[np.argmax(acq_fcn_vec)], '*', markersize=15, 
        #         label=u'Next Point to evaluate', markerfacecolor='gold', markeredgecolor='k', markeredgewidth=1)
        acq_axis_handle.set_xlim((x.min(), x.max()))
        acq_axis_handle.set_ylim((np.min(acq_fcn_vec) - (np.max(acq_fcn_vec) - np.min(acq_fcn_vec))*0.1, np.max(acq_fcn_vec) + (np.max(acq_fcn_vec) - np.min(acq_fcn_vec))*0.1))
        acq_axis_handle.set_ylabel('$acq_axis_handle \; fcn$', fontdict={'size':20})
        acq_axis_handle.set_xlabel('$x$', fontdict={'size':20})
        acq_axis_handle.tick_params(axis='x', labelsize=18)
        acq_axis_handle.tick_params(axis='y', labelsize=18)
        
        f_axis_handle.legend(loc=2, bbox_to_anchor=(1.01, 1), borderaxespad=0.)
        acq_axis_handle.legend(loc=2, bbox_to_anchor=(1.01, 1), borderaxespad=0.)

        if figure_handle == True:
            return fig, f_axis_handle, acq_axis_handle
